...
class RepositorioPerfilDispositivo {
  const RepositorioPerfilDispositivo();

  Future<PerfilDispositivo?> fetchProfile({
    ...
    return PerfilDispositivo.fromMap(Map<String, dynamic>.from(row));
  }

  Future<Map<String, PerfilDispositivo>> fetchProfilesByIds({
    required SupabaseClient client,
    required Iterable<String> deviceIds,
  }) async {
    final ids =
        deviceIds.map((e) => e.trim()).where((e) => e.isNotEmpty).toSet();
  ...
  Future<PerfilDispositivo> upsertProfile({
    required SupabaseClient client,
    required String deviceId,
    required String deviceLabel,
    required String referenceName,
    required ModoPublicoPerfilDispositivo publicMode,
    String? publicAlias,
  }) async {
    ...
    final row = await client
        ...
        .upsert({
          ...
          'public_alias': publicMode == ModoPublicoPerfilDispositivo.alias
  ...
  Future<PerfilDispositivo> ensureProfileShell({
    required SupabaseClient client,
    required String deviceId,
    required String deviceLabel,
  }) async {
    ...
    final row = await client
        ...
        .upsert({
          'device_id': deviceId,
          'device_label': deviceLabel.trim(),
          'public_mode': ModoPublicoPerfilDispositivo.anonymous.value,
          'updated_at': now,
          'created_at': now,
        }, onConflict: 'device_id')
        .select()
