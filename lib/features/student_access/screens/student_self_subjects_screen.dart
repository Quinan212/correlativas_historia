...
class StudentSelfSubjectsScreen extends ConsumerStatefulWidget {
  const StudentSelfSubjectsScreen({
    super.key,
  ...
  @override
  ConsumerState<StudentSelfSubjectsScreen> createState() =>
      _StudentSelfSubjectsScreenState();
}

class _StudentSelfSubjectsScreenState
    extends ConsumerState<StudentSelfSubjectsScreen> {
  final _repo = const StudentAccessRepository();
  bool _busy = false;
  late List<StudentAccessSubject> _allSubjects;
  ...
  Future<void> _loadPlan() async {
    final plan = await loadPlanFromHtmlAsset(
  ...
  bool _isSelfReported(StudentAccessSubject subject) {
    final officialIds = widget.payload.subjects.map((s) => s.subjectId).toSet();
    return !officialIds.contains(subject.subjectId);
  }

  Future<void> _deleteSubject(StudentAccessSubject subject) async {
  ...
  Future<void> _openEditor({StudentAccessSubject? existing}) async {
    final result = await Navigator.of(context).push<Map<String, dynamic>>(
      MaterialPageRoute(
        builder: (_) => _SelfSubjectEditorPage(
    ...
    try {
      ...
      setState(() {
        ...
        final updated = StudentAccessSubject(
          subjectId: result['subject_id'] as String,
          subjectName: result['subject_name'] as String,
          subjectYear: result['subject_year'] as int,
          status: result['status'] as String,
  ...
  Widget build(BuildContext context) {
    ...
    final grouped = <int, List<StudentAccessSubject>>{
      1: [],
      2: [],
      3: [],
      4: [],
                      ...
                      onTap: () {
                        Navigator.of(context).push(
                          MaterialPageRoute<void>(
                            builder: (context) => _YearSubjectsScreen(
                              year: year,
...
class _SelfSubjectCard extends StatelessWidget {
  const _SelfSubjectCard({
  ...
  final StudentAccessSubject subject;
                        ...
                        Wrap(
                          ...
                          children: [
                            ...
                            if (subject.academicPeriod.isNotEmpty &&
                              ...
                              _MiniChip(
                                label: _periodLabel(subject.academicPeriod),
...
class _YearSubjectsScreen extends StatelessWidget {
  const _YearSubjectsScreen({
    required this.year,
  ...
  final int year;
  final List<StudentAccessSubject> subjects;
  final List<StudentAccessSubject> allSubjects;
  final List<Materia> plan;
  final StudentAccessPayload payload;
  final Future<void> Function(StudentAccessSubject) onDelete;
  final Future<void> Function({StudentAccessSubject? existing}) onEdit;
  final bool busy;
  final bool Function(StudentAccessSubject) isSelfReported;
...
class _SelfSubjectEditorPage extends StatefulWidget {
  const _SelfSubjectEditorPage({
  ...
  final List<Materia> plan;
  final StudentAccessSubject? existing;
  final Set<String> existingSubjectIds;
  final List<StudentAccessSubject> allSubjects;
  final int studentYear;

  @override
  State<_SelfSubjectEditorPage> createState() => _SelfSubjectEditorPageState();
}

class _SelfSubjectEditorPageState extends State<_SelfSubjectEditorPage> {
  int _step = 0;
  ...
  // ---- Correlatividades helpers (same logic as student_access_screen.dart) ----
  ...
  bool _cumpleCorrelativas(Materia materia) {
    ...
    final byId = <String, StudentAccessSubject>{};
  ...
  Widget _buildReqRow(
      ...
      Map<String, StudentAccessSubject> subjectMap,
      ThemeData theme,
      ColorScheme cs) {
