...
String _displayNameForScreen(MateriaParaLista item) {
  final raw =
      sanitizeText(item.materiaPlan?.displayNombre ?? item.nombreBase).trim();
...
Widget _daysBadge(BuildContext context, DateTime? dt) {
  final theme = Theme.of(context);
  final isDark = theme.brightness == Brightness.dark;
  final info = _tiempoRelativo(dt);
  final text = _daysBadgeTextCompact(dt);
...
String _daysBadgeTextCompact(DateTime? dt) {
...
String _daysLabelCompact(DateTime? dt) {
...
String _daysBadgeText(DateTime? dt) {
  if (dt == null) return 'SIN\nFECHA';
