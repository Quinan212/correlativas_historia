...
class ExamenesJerarquicoHome extends ConsumerStatefulWidget {
  const ExamenesJerarquicoHome({super.key});

  static const _careers = ['historia', 'geografia', 'politica'];

  @override
  ConsumerState<ExamenesJerarquicoHome> createState() =>
      _ExamenesJerarquicoHomeState();
}

class _ExamenesJerarquicoHomeState
    extends ConsumerState<ExamenesJerarquicoHome> {
  int? _yearFilter;

  List<int> _availableYearsForCareer(String careerId) {
    final eventos = ref.watch(examenesAllProvider).valueOrNull;
  ...
  Widget build(BuildContext context) {
    ...
    final careers = kCareers
        .where((c) => ExamenesJerarquicoHome._careers.contains(c.id))
        .toList();

    return RefreshIndicator(
      onRefresh: () async {
        ref.invalidate(examenesAllProvider);
        await ref.read(examenesAllProvider.future);
      },
      child: SafeArea(
        top: false,
        bottom: true,
            ...
            SliverToBoxAdapter(
              child: Padding(
                ...
                child: Column(
                  ...
                  children: [
                    _ModeSwitcher(
                      selected: ExamenesViewMode.jerarquico,
                      onChanged: (value) {
                        ref.read(examenesViewModeProvider.notifier).state =
                            value;
                      },
                    ),
                        ...
                        onTap: () {
                          Navigator.of(context).push(
                            MaterialPageRoute<void>(
                              builder: (_) => _JerarquicoYearScreen(
                                careerId: careerId,
                                year: year,
                              ),
                            ),
...
class _ModeSwitcher extends StatelessWidget {
  ...
  final ExamenesViewMode selected;
  final ValueChanged<ExamenesViewMode> onChanged;
...
class _YearLaunchCard extends StatelessWidget {
  const _YearLaunchCard({
    required this.year,
    required this.onTap,
  });
...
class _JerarquicoYearScreen extends StatelessWidget {
  const _JerarquicoYearScreen({
        ...
        children: [
          _ScopeLaunchCard(
            ...
            onTap: () {
              Navigator.of(context).push(
                MaterialPageRoute<void>(
                  builder: (_) => _JerarquicoScopeScreen(
                    careerId: careerId,
                    year: year,
                    scope: 'llamados',
                  ),
...
class _ScopeLaunchCard extends StatelessWidget {
  const _ScopeLaunchCard({
    required this.title,
...
class _JerarquicoScopeScreen extends ConsumerStatefulWidget {
  const _JerarquicoScopeScreen({
    required this.careerId,
    required this.year,
    required this.scope,
  });
  ...
  @override
  ConsumerState<_JerarquicoScopeScreen> createState() =>
      _JerarquicoScopeScreenState();
}

class _JerarquicoScopeScreenState
    extends ConsumerState<_JerarquicoScopeScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final planMapaAsync = ref.watch(planMapaMateriasProvider(widget.careerId));
