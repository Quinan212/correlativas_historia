...
class AdminExamEvent {
  ...
  factory AdminExamEvent.fromRow(Map<String, dynamic> row) {
    ...
    return AdminExamEvent(
      ...
      careerId: sanitizeText((row['career_id'] ?? '').toString().trim()),
      anio: parseInt(row['anio']),
      fecha: parseDate(row['fecha']),
      hora: parseHora(row['hora']),
      materia: sanitizeText((row['materia'] ?? '').toString().trim()),
      instancia:
          sanitizeText((row['instancia'] ?? 'llamado_1').toString().trim()),
      docentes: parseDocentes(row['docentes']),
      actaUrl: sanitizeText((row['acta_url'] ?? '').toString().trim()).isEmpty
          ? null
          : sanitizeText(row['acta_url'].toString().trim()),
    );
  }
}

class AdminExamEventDraft {
  const AdminExamEventDraft({
    this.id,
    required this.careerId,
    required this.anio,
    required this.fecha,
