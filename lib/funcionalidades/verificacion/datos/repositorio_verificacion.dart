...
class RepositorioVerificacion {
  ...
  Future<SolicitudVerificacion> submitRequest({
    ...
    try {
      inserted = await client
          ...
          .insert({
            ...
            'image_path': path,
            'image_url': publicUrl,
            'status': EstadoSolicitudVerificacion.pending.value,
          })
          .select()
          .single();
    } catch (error) {
    ...
    final request = SolicitudVerificacion.fromMap(inserted);
  ...
  Future<List<SolicitudVerificacion>> fetchOwnRequests({
    required SupabaseClient client,
    required String deviceId,
  }) async {
    ...
    return rows
        ...
        .map(SolicitudVerificacion.fromMap)
        .toList(growable: false);
  }

  Stream<List<SolicitudVerificacion>> watchOwnRequests({
    required SupabaseClient client,
    required String deviceId,
  }) {
    return client
  ...
  Future<List<SolicitudVerificacion>> fetchPendingRequests({
    required SupabaseClient client,
  }) async {
    final rows = await client
        .from('verification_requests')
        .select()
        .eq('status', EstadoSolicitudVerificacion.pending.value)
  ...
  Stream<List<SolicitudVerificacion>> watchPendingRequests({
    required SupabaseClient client,
  }) {
    return client
        .from('verification_requests')
  ...
  Stream<List<SolicitudVerificacion>> watchReviewedRequests({
    required SupabaseClient client,
  }) {
    return client
        .from('verification_requests')
        ...
        .map(
          (rows) => rows
              ...
              .map(SolicitudVerificacion.fromMap)
              .where(
                  (item) => item.status != EstadoSolicitudVerificacion.pending)
              .toList(growable: false),
        );
  }
  ...
  Future<void> approveRequest({
    required SupabaseClient client,
    required SolicitudVerificacion request,
    required String adminDeviceId,
  }) async {
    final now = DateTime.now().toUtc().toIso8601String();

    await client.from('verification_requests').update({
      'status': EstadoSolicitudVerificacion.approved.value,
  ...
  Future<void> rejectRequest({
    ...
    await client.from('verification_requests').update({
      'status': EstadoSolicitudVerificacion.rejected.value,
