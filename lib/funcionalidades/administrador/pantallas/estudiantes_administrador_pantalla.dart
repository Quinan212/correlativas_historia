                      ...
                      children: [
                        ...
                        SizedBox(
                          ...
                          child: effectiveSelected == null
                              ? const _SinEstudianteSeleccionado()
                              : _PanelAcademicoEstudiante(
...
class _TarjetaFormularioEstudiante extends StatelessWidget {
  const _TarjetaFormularioEstudiante({
    required this.busy,
...
class _TarjetaCargaMasiva extends StatelessWidget {
  const _TarjetaCargaMasiva({
    required this.busy,
...
class _TablaEstudiantes extends StatelessWidget {
  const _TablaEstudiantes({
            ...
            children: [
              ...
              Align(
                ...
                child: SizedBox(
                  ...
                  child: Align(
                    alignment: Alignment.centerRight,
                    child: _EtiquetaEstadoEstudiante(student: student),
                  ),
                ),
              ),
            ],
...
class _EtiquetaEstadoEstudiante extends StatelessWidget {
  const _EtiquetaEstadoEstudiante({required this.student});
...
class _SinEstudianteSeleccionado extends StatelessWidget {
  const _SinEstudianteSeleccionado();
...
class _PanelAcademicoEstudiante extends ConsumerStatefulWidget {
  const _PanelAcademicoEstudiante({
    required this.adminDeviceId,
    required this.student,
  });
  ...
  @override
  ConsumerState<_PanelAcademicoEstudiante> createState() =>
      _PanelAcademicoEstudianteState();
}

class _PanelAcademicoEstudianteState
    extends ConsumerState<_PanelAcademicoEstudiante> {
  ...
  @override
  void didUpdateWidget(covariant _PanelAcademicoEstudiante oldWidget) {
                  ...
                  Padding(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        Text(
                          'DNI ${widget.student.dni} · ${_etiquetaCarrera(widget.student.careerId)} · ${_etiquetaAnio(widget.student.currentYear)}'
                  ...
                  Expanded(
                    child: TabBarView(
                      children: [
                        ...
                        _TabHistorial(historyAsync: historyAsync),
                      ],
                    ),
                  ),
                ],
  ...
  Future<void> _openSubjectEditor(
    ...
    final draft = await showDialog<BorradorMateriaEstudianteAdministrador>(
      context: context,
      builder: (_) => _DialogoEditorMateria(
...
class _TabMateriaMasiva extends StatelessWidget {
  const _TabMateriaMasiva({
    required this.year,
                  ...
                  ...roster.map(
                    (item) => Padding(
                      ...
                      child: CheckboxListTile(
                        ...
                        subtitle: Text(
                          'DNI ${item.dni} · ${_etiquetaAnio(item.currentYear)}${item.division == null ? '' : ' · ${item.division}'} · ${_etiquetaEstado(item.subject.status)}${item.subject.grade == null ? '' : ' · Nota ${item.subject.grade!.toStringAsFixed(0)}'}',
                          maxLines: 2,
                          overflow: TextOverflow.ellipsis,
                        ),
                      ),
...
class _TabHistorial extends StatelessWidget {
  const _TabHistorial({required this.historyAsync});
...
class _TarjetaResumenInscripcion extends StatelessWidget {
  const _TarjetaResumenInscripcion({
...
class _BloqueMateriasAnio extends StatelessWidget {
  const _BloqueMateriasAnio({
    required this.year,
    required this.materias,
    required this.savedBySubjectId,
    required this.selectedSubjectIds,
  ...
  String _subtituloMateria(
    ...
    final parts = [
      _etiquetaEstado(saved.status),
      _etiquetaCondicion(saved.conditionStatus),
      if (saved.detailStatus != null) _etiquetaDetalle(saved.detailStatus!),
      if (saved.academicPeriod != null) _etiquetaPeriodo(saved.academicPeriod!),
...
class _DialogoEditorMateria extends StatefulWidget {
  const _DialogoEditorMateria({
    required this.student,
  ...
  @override
  State<_DialogoEditorMateria> createState() => _DialogoEditorMateriaState();
}

class _DialogoEditorMateriaState extends State<_DialogoEditorMateria> {
  late String _status;
...
String _etiquetaEstado(String value) {
  return switch (value) {
    'regular' => 'Regular',
    'aprobada' => 'Aprobada',
    'no_regularizada' => 'No regularizada',
...
String _etiquetaCondicion(String value) {
  return switch (value) {
    'condicional' => 'Condicional',
    'bloqueada' => 'Bloqueada',
    _ => 'Habilitada',
...
String _tituloHistorial(EntradaHistorialEstudianteAdministrador entry) {
  ...
  if (subjectName.isNotEmpty) {
    ...
    final statusLabel = status.isEmpty ? '' : ' · ${_etiquetaEstado(status)}';
...
String _subtituloHistorial(EntradaHistorialEstudianteAdministrador entry) {
  final parts = <String>[
    ...
    if ((entry.payload['academic_period'] ?? '').toString().trim().isNotEmpty)
      _etiquetaPeriodo(entry.payload['academic_period'].toString()),
    if ((entry.payload['detail_status'] ?? '').toString().trim().isNotEmpty)
      _etiquetaDetalle(entry.payload['detail_status'].toString()),
...
class _TarjetaPanel extends StatelessWidget {
  const _TarjetaPanel({required this.child});
