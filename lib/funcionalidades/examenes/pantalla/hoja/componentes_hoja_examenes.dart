...
class DatosTabInstancia {
  factory DatosTabInstancia.fromEventos({
    required String id,
    required String label,
    required String materia,
    required List<EventoExamen> eventos,
  }) {
    final options = <DatosOpcionDivision>[];
    for (var i = 0; i < eventos.length; i++) {
      final evento = eventos[i];
      final divisionLabel = _etiquetaDivisionParaEvento(
        materiaBase: materia,
    ...
    return DatosTabInstancia(id: id, label: label, options: options);
  }

  const DatosTabInstancia({
    required this.id,
    required this.label,
  ...
  final String id;
  final String label;
  final List<DatosOpcionDivision> options;
}

class DatosOpcionDivision {
  const DatosOpcionDivision({
    required this.id,
    required this.label,
...
String _etiquetaDivisionParaEvento({
...
class PanelExamenMateria extends StatelessWidget {
  ...
  final List<DatosTabInstancia> tabs;
  ...
  DatosTabInstancia get _activeTab {
    if (tabs.isEmpty) {
      return const DatosTabInstancia(
        id: 'none',
        label: 'Sin datos',
        options: <DatosOpcionDivision>[],
      );
    }
  ...
  DatosOpcionDivision? get _activeOption {
  ...
  String _etiquetaHora(DateTime? dt, {required bool esColoquio}) {
  ...
  Future<void> _copyInfo(
    ...
    DatosTabInstancia tab,
    DatosOpcionDivision? option,
  ) async {
    ...
    final lines = <String>[
      ...
      'Hora: ${_etiquetaHora(dt, esColoquio: esColoquio)}',
