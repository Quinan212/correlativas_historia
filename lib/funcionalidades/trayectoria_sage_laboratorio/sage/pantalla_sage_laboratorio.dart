...
class _PantallaSageLaboratorioState
    extends State<PantallaSageLaboratorio> {
  ...
  void _completeProfileSelection(
    PerfilSage profile,
    int transitionId,
  ) {
    if (!mounted || transitionId != _profileTransitionId) return;
        ...
        if (_effectiveProfile != PerfilSage.alumnos) {
          ...
          widget.onEstadoPreparacion?.call(
            EstadoPreparacionSageLaboratorio(
              mensaje: 'Historial detectado. Preparando ${careers.length} carrera(s)…',
              progreso: 0,
            ),
          );
  ...
  Future<void> _autoLoadAllCareers(
    List<CarreraHistorialSage> careers,
  ) async {
    if (!mounted ||
    ...
    try {
      for (var index = 0; index < careers.length; index++) {
        ...
        if (_historyAutoAttemptedCareerIds.contains(current.gridRowId)) {
          widget.onEstadoPreparacion?.call(
            EstadoPreparacionSageLaboratorio(
              mensaje: 'La lectura de ${current.nombre} quedó incompleta. Tocá actualizar dentro de Historial para reintentar.',
              bloqueado: true,
            ),
          );
          return;
        ...
        widget.onEstadoPreparacion?.call(
          EstadoPreparacionSageLaboratorio(
            mensaje: 'Leyendo ${current.nombre} (${index + 1}/${careers.length})…',
        ...
        if (result.estado != EstadoCargaMateriasSage.disponible &&
          ...
          widget.onEstadoPreparacion?.call(
            EstadoPreparacionSageLaboratorio(
              mensaje: 'No se pudo completar ${current.nombre}. Volvé a intentarlo desde Historial.',
              bloqueado: true,
            ),
          );
          return;
  ...
  void _emitTrajectoryIfReady() {
    ...
    widget.onEstadoPreparacion?.call(
      EstadoPreparacionSageLaboratorio(
        mensaje: '${trajectory.totalMaterias} materias listas para sincronizar.',
        progreso: 1,
      ),
    );
    ScaffoldMessenger.of(context)
      ...
      ..showSnackBar(
        SnackBar(
          behavior: SnackBarBehavior.floating,
          content: Text(
            '${trajectory.totalMaterias} materias preparadas.',
          ),
...
Widget _buildCurrentNativeSageScreen() {
  ...
  return KeyedSubtree(
    ...
    child: PantallaCargaSage(
      mensaje: _navigationActionInFlight == null &&
              _legajoActionInFlight == null
