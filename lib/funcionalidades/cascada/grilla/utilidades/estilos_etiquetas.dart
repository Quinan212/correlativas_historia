...
(Color bg, Color fg, Color bd) coloresTipo(bool isDark, String tipo) {
  ...
  final isAdmin = s.contains('administración') ||
      ...
      s.contains('admin');
  final isMate = s.contains('matemática') ||
      s.contains('matematica') ||
      s.contains('mate');
  final isFlexible = s.contains('flexible');
  final isHumanistica = s.contains('humanística') ||
      s.contains('humanistica') ||
      s.contains('humani');
  ...
  if (isContable) {
    return (
      oscurecer(const Color(0xFF064E3B)),
      const Color(0xFFA7F3D0),
      const Color(0xFF047857)
    );
  }
  if (isJuridica) {
    return (
      oscurecer(const Color(0xFF7F1D1D)),
      const Color(0xFFFECACA),
  ...
  if (isEconomia) {
    return (
      oscurecer(const Color(0xFF1E3A8A)),
      const Color(0xFFBFDBFE),
  ...
  if (isFlexible) {
    return (
      oscurecer(const Color(0xFF155E75)),
      const Color(0xFFA5F3FC),
  ...
  if (isHumanistica) {
    return (
      oscurecer(const Color(0xFF831843)),
      const Color(0xFFFBCFE8),
